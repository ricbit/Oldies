...
STAKMAIN        = 400h          ; main execution stream stack size (in para)
STAKRMODE       = 80h           ; real mode call stack size (in para)
STAKPMODE       = 100h          ; protected mode call stack size (in para)
MODENESTING     = 15            ; max number of nested mode switches

RMODENUM        = (MODENESTING+1) shr 1
PMODENUM        = MODENESTING shr 1
STAKSIZE        = STAKMAIN+(PMODENUM*STAKPMODE)+(RMODENUM*STAKRMODE)

DETECTORDER     = 1             ; 0 = DPMI then VPCI, 1 = VCPI then DPMI
...
start16:                                ; Program begins here
	...
	mov ds:_code16a,eax
        ...
        int     21h
        mov     es,bx
        movzx   cx,byte ptr es:[080h]
        cmp     cx,0
        je      fudeb_out
        mov     bx,80h
        inc     cx
        mov     edi,offset _cmdline
        mov     cx,081h
fudeba_loop:
        push    cx
        push    bx
...
d_nintoff       dd      offset dp_irq0,offset dp_irq1,offset dp_irq2,offset dp_irq3
		...
		dd      offset dp_int33,offset dp_int32
;±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±
; 16 bit DPMI system code
;±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±
;ÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍ
d_retreal:                              ; Return to real mode
	mov ax,205h                     ; restore all int vectors needed
	mov edi,17
d_retreall0:
...
v_start:                                ; start continues from VCPI
	...
	mov c_idt32handler[40h],offset vp_int33 ; VCPI safe int handlers
	mov c_idt32handler[44h],offset vp_int32
	mov ds:cp_v86irqintr,offset vp_int33f0  ; VCPI IRQ safe int routine
...
cp_v86irqintr   dd      offset cp_int33f0
