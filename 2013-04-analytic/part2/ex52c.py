# Alignments with no singleton cycles
# usage: python ex52c.py 2 | dot -Kcirco -Tpng -oi52c1.png
...
def set_partitions_no_singletons(n):
  for part in set_partitions(n):
    if all(len(p) > 1 for p in part):
      yield part
...
n = int(sys.argv[1])
code = 0
print "digraph alignments {"
for partition in set_partitions_no_singletons(n):
  for cycle in itertools.permutations(xrange(len(partition))):
    for outer in subcycle([partition[i] for i in cycle]):
      for i, inner in enumerate(outer):
        ...
        if len(outer) > 1 and i < len(outer) - 1:
